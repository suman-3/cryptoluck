...
export default function FooterSection() {
    return (
        <div className="mx-auto max-w-screen-2xl w-full mt-4">
            <div className="grid gap-12 md:grid-cols-5">
                <div className="md:col-span-2 flex items-start flex-col justify-start gap-4">
                    <Link href="/" aria-label="go home" className="block size-fit">
                        <Image src='/logo.svg' alt='logo' width={100} height={100} className='w-14 h-auto' />
                    </Link>
                    <p className='text-sm font-normal tracking-wide text-white w-80'>
                        The blockchain-powered lottery platform that brings transparency, security, and excitement to players worldwide
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6 sm:grid-cols-4 md:col-span-3">
                    {links.map((link, index) => (
                        <div key={index} className="space-y-4 text-sm">
                            <span className="block font-medium text-lg text-white">{link.group}</span>
                            {link.items.map((item, index) => (
                                <Link key={index} href={item.href} className="text-muted-foreground hover:text-primary block duration-150">
                                    <span className='text-white hover:text-slate-100 transition'>{item.title}</span>
                                </Link>
                            ))}
                        </div>
                    ))}
            ...
            <div className="mt-12 flex flex-wrap items-end justify-between gap-6 border-t py-6">
                <span className="text-muted-foreground order-last block text-center text-sm md:order-first">© {new Date().getFullYear()} CryptoLuck, All rights reserved</span>
