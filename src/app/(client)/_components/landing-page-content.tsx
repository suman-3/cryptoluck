...
const LandingPageContent = () => {
    ...
    return (
        <MaxWidthWrapper className='flex flex-col gap-7 relative max-h-screen min-h-screen w-full overflow-x-hidden'>
            {/* heo section */}
            <section className='w-full flex items-center gap-4 pt-14'>
                <div className='w-1/2 flex flex-col gap-6 pl-6 items-start relative'>
                    <h1 className='text-white text-6xl font-bold tracking-wide leading-[5rem]'>
                        Welcome to <br />
                        CryptoLuck
                    </h1>
                    ...
                    <div className='flex items-center gap-4 mt-4'>
                        <Button className='w-48 !h-14 rounded-full bg-[#6368E9] hover:bg-[#6368E9] text-white font-normal tracking-wide cursor-pointer group text-md'>
                            Buy A Ticket
                            <MoveRight className='size-5 shrink-0 group-hover:translate-x-1 transition' />
                        </Button>
                        <Button variant='ghost' className='text-white w-40 cursor-pointer !h-14 rounded-full border border-[#6368E9] hover:bg-transparent hover:text-white'>
                            View Contest
                        </Button>
                    </div>
                    <div className="absolute -bottom-10 -left-8 z-20 hidden md:block">
                        <AnimatedCursor text={"anonymouse"} />
                    </div>
                </div>
                <div className='w-1/2 flex justify-center items-center'>
                    <Image src='/hero.svg' alt='landing page image' width={600} height={600} className='w-[85%] h-auto z-10 select-none pointer-events-none' />
                </div>
            </section>
            <Features />
            <Banners />
